...
EContainer::EContainer(QWidget *parent, QPoint p, QWidget *cWidget) : QWidget(parent) {
    mode = NONE;
    childWidget = cWidget;
...
void EContainer::setChildWidget(QWidget *cWidget) {
    if (cWidget != 0) {
        childWidget = cWidget;
...
void EContainer::popupShow(const QPoint &pt) {
...
void EContainer::focusInEvent(QFocusEvent *) {
    m_infocus = true;
...
void EContainer::focusOutEvent(QFocusEvent *) {
...
bool EContainer::eventFilter(QObject *obj, QEvent *evt) {
    if (m_infocus) {
...
void EContainer::mousePressEvent(QMouseEvent *e) {
...
void EContainer::keyPressEvent(QKeyEvent *e) {
    if (!m_isEditing) return;
...
void EContainer::setCursorShape(const QPoint &e_pos) {
    const int diff = 3;
    if (
            //Left-Bottom
...
void EContainer::mouseReleaseEvent(QMouseEvent *e) {
    QWidget::mouseReleaseEvent(e);
}

void EContainer::mouseMoveEvent(QMouseEvent *e) {
    ...
    if ((mode == MOVE || mode == NONE) && e->buttons() && Qt::LeftButton) {
        ...
        if(m_moveEnable)move(toMove);
        emit newGeometry(this->geometry());
        this->parentWidget()->repaint();
        return;
    }
    if ((mode != MOVE) && e->buttons() && Qt::LeftButton) {
        if((m_modeEnable!=NONE)&&(m_modeEnable!=mode))return;
        switch (mode){
